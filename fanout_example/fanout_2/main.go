...
import (
  ...
  "fmt"
	...
	"github.com/hanapedia/microservice-topologies/fanout_example/fanout_2/connections/mongo"
	pb "github.com/hanapedia/microservice-topologies/fanout_example/fanout_2/pb_fanout_2"
...
func (s fanout_2Server) GetIds(ctx context.Context, req *pb.Req) (*pb.Res, error) {
	...
	if err != nil {
		log.Fatalf("Failed to retrieve item at fanout_2: %v", err)
	}
	returnArr := req.Ids
...
func main() {
  ...
  lis, err := net.Listen("tcp", fmt.Sprintf(":%s", listenPort))
	...
	mongoConnection, err = mongo.InitMongo(dbAddress, dbName, "fanout2")
	if err != nil {
