...
import (
	...
	"github.com/hanapedia/microservice-topologies/chain/chain_3/connections/mongo"
	pb "github.com/hanapedia/microservice-topologies/chain/chain_3/pb_chain_3"
	pbc "github.com/hanapedia/microservice-topologies/chain/chain_3/pb_chain_4"
...
var nextChain pbc.Chain_4Client
...
func newServer() chain_3Server {
  return chain_3Server{}
}
...
func main() {
  ...
  pb.RegisterChain_3Server(grpcServer, newServer())
  ...
  optsClient = append(optsClient, grpc.WithTransportCredentials(insecure.NewCredentials()))
    ...
    conn, err := grpc.Dial("localhost:3004", optsClient...)
  if err != nil {
    log.Printf("Cannot establish connection with the server: %v", err)
  }
  defer conn.Close()

  nextChain = pbc.NewChain_4Client(conn)
